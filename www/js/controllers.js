...
.controller('ChatDetailCtrl', ['$scope', 'Loader', '$ionicPlatform', '$stateParams', 'UserFactory', 'FBFactory', '$ionicScrollDelegate', '$cordovaImagePicker', 'Utils', '$timeout', '$ionicActionSheet', '$cordovaCapture', '$cordovaGeolocation', '$ionicModal',
    function ($scope, Loader, $ionicPlatform, $stateParams, UserFactory, FBFactory, $ionicScrollDelegate, $cordovaImagePicker, Utils, $timeout, $ionicActionSheet, $cordovaCapture, $cordovaGeolocation, $ionicModal) {
        $ionicPlatform.ready(function () {
            Loader.show('Estabelecendo conexão...');
            ...
            $scope.sendMessage = function () {
                ...
                var msg = '<p>' + $scope.user.cachedUserProfile.name + ' disse : <br/>' + $scope.chatMsg + '</p>';
                var type = 'text';
                                ...
                                .then(function (results) {
                                    if(results.length > 0) {
                                        ...
                                        Utils.getBase64ImageFromInput(imageData, function (err, base64Img) {
                                            ...
                                            postMessage('<p>' + $scope.user.cachedUserProfile.name + ' postou : <br/><img class="chat-img" src="' + base64Img + '">', 'img');
                                            Loader.hide();
                                        });
                                    }
                                }, function (error) {
            ...
            $scope.centerOnMe = function () {
                ...
                $cordovaGeolocation.getCurrentPosition(posOptions).then(function (pos) {
                    ...
                    alert('O aplicativo não pode te localizar, habilite o GPS do seu aparelho para continuar');
                    Loader.hide();
                    $scope.modal.hide();
                });
            };
            ...
            $scope.selectLocation = function () {
                ...
                postMessage('<p>' + $scope.user.cachedUserProfile.name + ' compartilhou : <br/>', type, map);
                $scope.modal.hide();
            }
